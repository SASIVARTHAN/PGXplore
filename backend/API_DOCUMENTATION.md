...
### Send Login OTP
- **Purpose:** Send a one-time password to a registered phone number
- **Method + URL:** `POST /api/auth/otp/send`
- **Auth:** None
- **Request:** `{ "phone": "9876543213" }`
- **Response:** `{ "demoOtp": "123456", "message": "OTP sent successfully" }` (dev OTP included for testing)
- **Success:** 200 | **Errors:** 404 (phone not registered)

### Verify Login OTP
- **Purpose:** Verify OTP and sign in with phone
- **Method + URL:** `POST /api/auth/otp/verify`
- **Auth:** None
- **Request:** `{ "phone": "9876543213", "otp": "123456", "portal": "user" }` (`portal` may be `owner` for PG Owner sign-in)
- **Success:** 200 | **Errors:** 400 (invalid OTP), 404 (phone not registered)
