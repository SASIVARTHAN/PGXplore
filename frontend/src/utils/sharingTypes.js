...
export function sharingObjectToFormConfigs(sharing) {
  return sharingToEntries(sharing).map((entry) => ({
    ...
    totalBeds:
      entry.totalBeds ?? getDefaultTotalBeds(entry.type) ?? '',
  }))
}
...
export function validateSharingConfigs(configs) {
  ...
  for (const config of configs) {
    ...
    if (config.totalBeds !== '' && config.totalBeds != null) {
      const beds = Number(config.totalBeds)
      if (!Number.isFinite(beds) || beds < 1) {
        return { valid: false, message: `Total beds must be at least 1 for ${label}.` }
      }
    }
  }
