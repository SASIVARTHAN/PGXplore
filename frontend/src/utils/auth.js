...
export function staffAccounts() {
  return AUTH_ACCOUNTS.filter((a) => a.role !== ROLES.NORMAL).map((account) => {
    const rest = { ...account }
...
/** Standard /login accepts only backend USER and PG_OWNER accounts. */
export function isUserPortalBackendRole(backendRole) {
  return backendRole === 'USER' || backendRole === 'PG_OWNER'
}
