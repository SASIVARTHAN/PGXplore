    ...
    onChange(
      value.map((c) => {
        ...
        if (patch.type && patch.type !== c.type) {
          const defaultBeds = getDefaultTotalBeds(patch.type)
          next.totalBeds = defaultBeds ?? ''
        }
        return next
      }),
    )
              ...
              <div className="sharing-config-card__grid">
                ...
                <label className="block text-sm sm:col-span-2">
                  <span className="font-medium text-main">
                    Total beds <span className="font-normal text-muted">(optional)</span>
                  </span>
                  <input
                    type="number"
                    min="1"
                    className="input-app mt-1 w-full sm:max-w-xs"
                    placeholder={
                      config.type && getDefaultTotalBeds(config.type) != null
                        ? `Default: ${getDefaultTotalBeds(config.type)}`
                        : 'e.g. 12 for dormitory'
                    }
                    value={config.totalBeds}
                    onChange={(e) => updateConfiguration(config.id, { totalBeds: e.target.value })}
                  />
                </label>
              </div>
            </li>
